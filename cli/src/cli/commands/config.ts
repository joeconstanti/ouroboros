...
export async function showConfig(workDir = process.cwd()): Promise<void> {
	if (!isInitialized(workDir)) {
		logWarn("No config found. Run 'ouroboros --init' first.");
		return;
	}
	...
	console.log("");
	console.log(`${pc.bold("Ouroboros Configuration")} (${configPath})`);
	console.log("");
	...
	if (config.rules.length > 0) {
		...
		console.log(`  ${pc.dim('(none - add with: ouroboros --add-rule "...")')}`);
	}
	console.log("");
...
export async function addRule(rule: string, workDir = process.cwd()): Promise<void> {
	if (!isInitialized(workDir)) {
		logError("No config found. Run 'ouroboros --init' first.");
		return;
	}
