...
# ============================================
# Ouroboros - Agentic Coding Loop
# OpenCode-first (supports Claude Code, Codex, Cursor, Qwen-Code and Factory Droid)
...
# Ouroboros config directory
OUROBOROS_DIR=".ouroboros"
PROGRESS_FILE="$OUROBOROS_DIR/progress.txt"
CONFIG_FILE="$OUROBOROS_DIR/config.yaml"
SINGLE_TASK=""
INIT_MODE=false
SHOW_CONFIG=false
ADD_RULE=""
...
SKIP_TESTS=false
SKIP_LINT=false
AI_ENGINE="opencode"  # opencode, claude, cursor, codex, qwen, or droid
DRY_RUN=false
...
# ============================================
# BROWNFIELD MODE (.ouroboros/ configuration)
# ============================================

# Initialize .ouroboros/ directory with config files
init_ouroboros_config() {
  if [[ -d "$OUROBOROS_DIR" ]]; then
    log_warn "$OUROBOROS_DIR already exists"
...
# https://github.com/fromtheroot/ouroboros
...
EOF
  ...
  # Create progress.txt
  echo "# Ouroboros Progress Log" > "$PROGRESS_FILE"
  echo "" >> "$PROGRESS_FILE"

  log_success "Created $OUROBOROS_DIR/"
  echo ""
  echo "  ${CYAN}$CONFIG_FILE${RESET}   - Your rules and preferences"
  echo "  ${CYAN}$PROGRESS_FILE${RESET} - Progress log (auto-updated)"
  echo ""
  echo "${BOLD}Next steps:${RESET}"
  echo "  1. Add rules:  ${CYAN}ouroboros --add-rule \"your rule here\"${RESET}"
  echo "  2. Or edit:    ${CYAN}$CONFIG_FILE${RESET}"
  echo "  3. Run:        ${CYAN}ouroboros \"your task\"${RESET} or ${CYAN}ouroboros${RESET} (with PRD.md)"
}

# Load rules from config.yaml
load_ouroboros_rules() {
...
# Show current config
show_ouroboros_config() {
  if [[ ! -f "$CONFIG_FILE" ]]; then
    log_warn "No config found. Run 'ouroboros --init' first."
    exit 1
  fi

  echo ""
  echo "${BOLD}Ouroboros Configuration${RESET} ($CONFIG_FILE)"
  echo ""
  ...
  if command -v yq &>/dev/null; then
    ...
    if [[ -n "$rules" ]]; then
      ...
      echo "  ${DIM}(none - add with: ouroboros --add-rule \"...\")${RESET}"
    fi
    echo ""
...
add_ouroboros_rule() {
  ...
  if [[ ! -f "$CONFIG_FILE" ]]; then
    log_error "No config found. Run 'ouroboros --init' first."
    exit 1
  fi
...
show_help() {
  cat << EOF
${BOLD}Ouroboros${RESET} - Agentic Coding Loop (v${VERSION})

${BOLD}USAGE:${RESET}
  ./ouroboros.sh [options]              # PRD mode (requires PRD.md)
  ./ouroboros.sh "task description"     # Single task mode (brownfield)
  ./ouroboros.sh --init                 # Initialize .ouroboros/ config

${BOLD}CONFIG & SETUP:${RESET}
  --init              Initialize .ouroboros/ with smart defaults
...
${BOLD}EXAMPLES:${RESET}
  ...
  ./ouroboros.sh --init                       # Initialize config
  ./ouroboros.sh "add dark mode toggle"       # Run single task
  ./ouroboros.sh "fix the login bug" --cursor # Single task with Cursor

  # PRD mode (task lists)
  ./ouroboros.sh                              # Run with OpenCode
  ./ouroboros.sh --codex                      # Run with Codex CLI
  ./ouroboros.sh --branch-per-task --create-pr  # Feature branch workflow
...
check_requirements() {
  ...
  if [[ ${#missing[@]} -gt 0 ]]; then
   ...
   if ! command -v git &>/dev/null; then
     log_error "git is required but not installed. Install git before running Ouroboros."
     exit 1
   fi
 
   # Check if we're in a git repository
   if ! git rev-parse --git-dir >/dev/null 2>&1; then
     log_error "Not a git repository. Ouroboros requires a git repository to track changes."
     exit 1
   fi
 
   # Ensure .ouroboros/ directory exists and create progress.txt if missing
   mkdir -p "$OUROBOROS_DIR"
...
create_task_branch() {
  local task=$1
  local branch_name="ouroboros/$(slugify "$task")"
  ...
  git stash push -m "ouroboros-autostash" >/dev/null 2>&1 || true
  stash_after=$(git stash list -1 --format='%gd %s' 2>/dev/null || true)
  if [[ -n "$stash_after" ]] && [[ "$stash_after" != "$stash_before" ]] && [[ "$stash_after" == *"ouroboros-autostash"* ]]; then
    stashed=true
  fi
...
create_pull_request() {
  local branch=$1
  local task=$2
  local body="${3:-Automated PR created by Ouroboros}"
...
notify_done() {
  local message="${1:-Ouroboros has completed all tasks!}"
  ...
  if command -v osascript &>/dev/null; then
    osascript -e "display notification \"$message\" with title \"Ouroboros\"" 2>/dev/null || true
  fi
  
  # Linux (notify-send)
  if command -v notify-send &>/dev/null; then
    notify-send "Ouroboros" "$message" 2>/dev/null || true
  fi
...
notify_error() {
  local message="${1:-Ouroboros encountered an error}"
  
  # macOS
  if command -v osascript &>/dev/null; then
    osascript -e "display notification \"$message\" with title \"Ouroboros - Error\"" 2>/dev/null || true
  fi
  
  # Linux
  if command -v notify-send &>/dev/null; then
    notify-send -u critical "Ouroboros - Error" "$message" 2>/dev/null || true
  fi
}
...
build_prompt() {
  ...
  # Add .ouroboros/ config if available (works with PRD mode too)
  if [[ -d "$OUROBOROS_DIR" ]]; then
    # Add project context
    local context
    context=$(load_project_context)
...
run_single_task() {
  ...
  while [[ $retry_count -lt $MAX_RETRIES ]]; do
    ...
    if [[ "$CREATE_PR" == true ]] && [[ -n "$branch_name" ]]; then
      create_pull_request "$branch_name" "$current_task" "Automated implementation by Ouroboros"
    fi
...
create_agent_worktree() {
  local task_name="$1"
  local agent_num="$2"
  local branch_name="ouroboros/agent-${agent_num}-$(slugify "$task_name")"
...
run_parallel_agent() {
  ...
  # Ensure .ouroboros/ and progress.txt exist in worktree
  mkdir -p "$worktree_dir/$OUROBOROS_DIR"
    ...
    if [[ "$CREATE_PR" == true ]]; then
      (
        ...
        gh pr create \
          ...
          --head "$branch_name" \
          --title "$task_name" \
          --body "Automated implementation by Ouroboros (Agent $agent_num)" \
...
run_parallel_tasks() {
  ...
  for group in "${groups[@]}"; do
    ...
    if [[ "$PRD_SOURCE" == "yaml" ]] && [[ ${#group_completed_branches[@]} -gt 0 ]] && [[ ${#groups[@]} -gt 1 ]]; then
      local integration_branch="ouroboros/integration-group-$group"
...
main() {
  ...
  if [[ -n "$SINGLE_TASK" ]]; then
    ...
    echo "${BOLD}============================================${RESET}"
    echo "${BOLD}Ouroboros${RESET} - Single Task Mode"
    ...
    if [[ -d "$OUROBOROS_DIR" ]]; then
      echo "Config: ${GREEN}$OUROBOROS_DIR/${RESET}"
    else
  ...
  # Show banner
  echo "${BOLD}============================================${RESET}"
  echo "${BOLD}Ouroboros${RESET} - Running until PRD is complete"
  local engine_display
  case "$AI_ENGINE" in
    opencode) engine_display="${CYAN}OpenCode${RESET}" ;;
    cursor) engine_display="${YELLOW}Cursor Agent${RESET}" ;;
  ...
  if [[ -d "$OUROBOROS_DIR" ]]; then
    echo "Config: ${GREEN}$OUROBOROS_DIR/${RESET} (rules loaded)"
  fi
  ...
  while true; do
    ...
    if [[ $MAX_ITERATIONS -gt 0 ]] && [[ $iteration -ge $MAX_ITERATIONS ]]; then
      log_warn "Reached max iterations ($MAX_ITERATIONS)"
      show_summary
      notify_done "Ouroboros stopped after $MAX_ITERATIONS iterations"
      exit 0
    fi
